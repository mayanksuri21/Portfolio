...
export function NeuralNoise({
  ...
  useEffect(() => {
    ...
    const resizeListener = () => resizeCanvas(canvasEl, gl);
    ...
    const render = () => {
      ...
      gl.uniform2f(uniformsRef.current.u_pointer_position, p.x / window.innerWidth, 1 - p.y / window.innerHeight);
  ...
  function initShader(canvasEl: HTMLCanvasElement) {
    const vsSource = `
  ...
  function resizeCanvas(canvasEl: HTMLCanvasElement, gl: WebGLRenderingContext) {
    const devicePixelRatio = Math.min(window.devicePixelRatio, 2);
